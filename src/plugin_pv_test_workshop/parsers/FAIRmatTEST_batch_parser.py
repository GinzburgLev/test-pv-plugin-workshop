...
from plugin_pv_test_workshop.schema_packages.FAIRmatTEST_package import (
    FAIRmatTEST_Batch,
    FAIRmatTEST_Cleaning,
    FAIRmatTEST_Evaporation,
    FAIRmatTEST_Inkjet_Printing,
    FAIRmatTEST_LaserScribing,
    FAIRmatTEST_Process,
    FAIRmatTEST_Sample,
    FAIRmatTEST_SlotDieCoating,
    FAIRmatTEST_SpinCoating,
    FAIRmatTEST_Sputtering,
    FAIRmatTEST_Substrate,
    FAIRmatTEST_AtomicLayerDeposition,
    ProcessParameter,
)
...
class RawFAIRmatTESTExperiment(EntryData):
...
class FAIRmatTESTExperimentParser(MatchingParser):
    def is_mainfile(
        self,
        filename: str,
        mime: str,
    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        archives = [map_batch(sample_ids, batch_id, upload_id, FAIRmatTEST_Batch)]
        substrates = []
        substrates_col = [
            'Sample dimension',
            'Sample area [cm^2]',
        ...
        for i, sub in df['Experiment Info'][substrates_col].drop_duplicates().iterrows():
            if pd.isna(sub).all():
                continue
            substrates.append((f'{i}_substrate', sub, map_substrate(sub, FAIRmatTEST_Substrate)))
        ...
        for i, row in df['Experiment Info'].iterrows():
            ...
            archives.append(map_basic_sample(row, substrate_name, upload_id, FAIRmatTEST_Sample))
        ...
        for i, col in enumerate(df.columns.get_level_values(0).unique()):
            ...
            for j, row in df_dropped.iterrows():
                ...
                if 'Cleaning' in col:
                    archives.append(map_cleaning(i, j, lab_ids, row, upload_id, FAIRmatTEST_Cleaning))

                if 'Laser Scribing' in col:
                    archives.append(map_laser_scribing(i, j, lab_ids, row, upload_id, FAIRmatTEST_LaserScribing))

                if 'Generic Process' in col:  # move up
                    generic_process = map_generic(i, j, lab_ids, row, upload_id, FAIRmatTEST_Process)
                ...
                if 'Evaporation' in col:
                    ...
                    if 'Co-Evaporation' in col:
                        coevap = True
                    archives.append(
                        map_evaporation(i, j, lab_ids, row, upload_id, FAIRmatTEST_Evaporation, coevap)
                    )

                if 'Spin Coating' in col:
                    archives.append(map_spin_coating(i, j, lab_ids, row, upload_id, FAIRmatTEST_SpinCoating))

                if 'Slot Die Coating' in col:
                    archives.append(map_sdc(i, j, lab_ids, row, upload_id, FAIRmatTEST_SlotDieCoating))

                if 'Sputtering' in col:
                    archives.append(map_sputtering(i, j, lab_ids, row, upload_id, FAIRmatTEST_Sputtering))

                if 'Inkjet Printing' in col:
                    archives.append(
                        map_inkjet_printing(i, j, lab_ids, row, upload_id, FAIRmatTEST_Inkjet_Printing)
                    )

                if 'ALD' in col:
                    archives.append(
                        map_atomic_layer_deposition(
                            i, j, lab_ids, row, upload_id, FAIRmatTEST_AtomicLayerDeposition)
                    )
        ...
        archive.data = RawFAIRmatTESTExperiment(processed_archive=refs)
