...
from plugin_pv_test_workshop.schema_packages.FAIRmatTEST_package import (
    FAIRmatTEST_EQEmeasurement,
    FAIRmatTEST_JVmeasurement,
    FAIRmatTEST_Measurement,
    FAIRmatTEST_SimpleMPPTracking,
)
...
class RawFileFAIRmatTEST(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class FAIRmatTESTParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = FAIRmatTEST_Measurement()

        if measurment_type == 'jv':
            entry = FAIRmatTEST_JVmeasurement()
        if measurment_type == 'eqe':
            entry = FAIRmatTEST_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = FAIRmatTEST_SimpleMPPTracking()
        ...
        archive.data = RawFileFAIRmatTEST(processed_archive=get_reference(archive.metadata.upload_id, eid))
