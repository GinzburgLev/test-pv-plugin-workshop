...
def set_monkey_patch(monkeypatch):
    ...
    monkeypatch.setattr(
        'plugin_pv_test_workshop.parsers.FAIRmatTEST_measurement_parser.set_sample_reference',
        mockreturn_search,
    )

    monkeypatch.setattr(
        'plugin_pv_test_workshop.schema_packages.FAIRmatTEST_package.set_sample_reference',
        mockreturn_search,
    )
